...
$$ t_e(f_e) = t_e^0 \left[1 + \alpha \left(\frac{f_e}{c_e}\right)^\beta \right] \enspace \text{with} \enspace \begin{cases} \text{$t_e^0$ free flow time} \\ \text{$c_e$ capacity}\end{cases}$$

Everyone behaves selfishly: find a Nash *equilibrium*.

*Continuous* optimization problem, requires solving many shortest paths.
...
*Discrete* optimization, requires solving many shortest paths.
